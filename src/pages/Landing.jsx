...
function Landing() {
  ...
  return (
    <div className="landing">
      ...
      {/* ───────────────────────────────────────────────────────────────
          SECTION 10 — Footer (Chat 6)
          Single horizontal strip: logo + wordmark (left), tagline
          (center), copyright + author credit (right). Border-top
          carries the visual seam between Section 9 and the footer
          chrome. Padding 32px vertical per kickoff.

          Logo SVG markup mirrors LandingNav.jsx exactly — same static
          square + SMIL-rotating line pair — so the top and bottom of
          the page open and close with the same brand mark. SMIL
          <animateTransform> is one of the three permitted Framer-Motion
          exceptions in Core Architecture Rules §4(c).

          Reveal: standard section reveal via revealVariants. Mobile
          (<768px): the three-region strip collapses to a stacked column
          (logo → tagline → copyright) — handled in CSS, no JSX change.
      ─────────────────────────────────────────────────────────────── */}
      <motion.footer
      ...
      >
        <div className="landing-footer-inner">
          {/* LEFT — logo + wordmark, internal link back to landing top.
              Same markup pattern as LandingNav's .landing-nav-brand
              cluster: static square SVG + rotor line SVG layered, plus
              the IBForge text node. CSS class names are footer-specific
              so they can carry their own sizing if needed later. */}
          <Link
            to="/"
          ...
          {/* CENTER — tagline. Locked copy per kickoff. */}
          <p className="landing-footer-tagline">
            Work counts when it exists.
          </p>

          {/* RIGHT — copyright + author credit, joined as one line for
              alignment but rendered as two semantic spans so the credit
              can break cleanly on narrow widths. */}
          <p className="landing-footer-copyright">
            <span className="landing-footer-copyright-year">
              © 2026 IBForge
            </span>
